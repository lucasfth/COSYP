...
/**
 * Compute the size of the data for each thread.
 * @param input_data The data to compute the size for.
 * @return The size of the data for each thread.
 */
int compute_size(std::vector<int> input_data) {
  return input_data.size() / NUM_THREADS;
}
...
int main() {
  ...
  vector<vector<int>> local_counts(NUM_THREADS, vector<int>(NUM_THREADS, 0)); // Local counts for each partition
  vector<int> global_offsets(NUM_THREADS, 0); // Global offsets for each partition
