...
perf_df = pd.read_csv("perf-metrics-bits.csv")

# Create subplots - FLIPPED order as requested
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

# Define algorithms and get unique bits counts
algorithms = ["concurrent-output", "count-then-move"]
all_bits = sorted(perf_df["bits"].unique())

# Custom colors for algorithms
colors = [
    "#1f77b4",
    "#ff7f0e",
]  # Blue for concurrent-output, Orange for count-then-move
...
ax1.set_xticks(all_bits)
ax1.set_xticklabels(all_bits)
...
plt.tight_layout()
plt.savefig("cache_misses_bits_comparison.png", bbox_inches="tight")
plt.show()
